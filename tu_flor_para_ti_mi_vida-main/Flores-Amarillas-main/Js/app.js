...
Boton1.addEventListener('click', function() {
    ...
    DibujarFlorBase(500, 100, 6, 30, 100, 200, true);
    h1.remove();
});
...
function DibujarTallo(x, y, AltoTrazo) {
    ...
    const DibujarTallo = () => {
        if (NuevaY < y + AltoTrazo) {
            ...
            ctx.stroke();
            NuevaY += AltoTallo;
            requestAnimationFrame(DibujarTallo); // Uso de requestAnimationFrame
        }
    };
    DibujarTallo();
}

function DibujarFlorBase(x, y, NumeroPetalos, RadioXPetalo, RadioYPetalo, AltoTrazo, conTallo = true) {
    if (conTallo) {
    ...
    function dibujarSiguientePetalo() {
        if (contadorPetalos <= NumeroPetalos) {
            ...
            DibujarPetalo(x, y, RadioXPetalo, 2, Angulo, 'yellow', 100); // Pétalos amarillos
            contadorPetalos++;
        ...
        ctx.fillStyle = '#8B4513'; // Marrón oscuro del centro
        ctx.fill();
    }
...
function CrearVarias() {
    ...
    for (let i = 0; i <= numFlores; i++) {
        ...
        DibujarFlorBase(x, y, 8, 30, 80, TamañoFlor, false); // Flor sin tallo
    }
}
